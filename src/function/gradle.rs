...
pub fn parse_dependencies_strings(gradle_output: &str) -> Fallible<Vec<String>> {
    let mut lines = gradle_output.lines();
    ...
    let mut list = Vec::from_iter(list.into_iter());
    list.sort();
...
fn pretty_name(line: &str) -> Option<String> {
    ...
    REG.captures(line)
        ...
        .map(|data| {
            ...
            let group_id = segments.get(0).expect("missing group id");
...
mod tests {
    ...
    #[test]
    fn parse_dependencies_strings_app_release_runtime_classpath() {
        let gradle_output = r#"
...
"#;
        ...
        //     .without_time()
        //     .init();
        let actual = parse_dependencies_strings(gradle_output).unwrap();
        let expected = vec![
    ...
    #[test]
    fn parse_dependencies_strings_app2_release_runtime_classpath() {
        let gradle_output = r#"
Type-safe project accessors is an incubating feature.
Project accessors enabled, but root project name not explicitly set for 'android-template'. Checking out the project in different folders will impact the generated code and implicitly the buildscript classpath, breaking caching.
...
"#;
    ...
    #[test]
    fn parse_dependencies_strings_release_runtime_classpath() {
        let gradle_output = r#"
releaseRuntimeClasspath - Runtime classpath of compilation 'release' (target  (androidJvm)).
+--- org.jetbrains.kotlin:kotlin-stdlib-jdk8:1.6.21
|    +--- org.jetbrains.kotlin:kotlin-stdlib:1.6.21 -> 1.7.10
